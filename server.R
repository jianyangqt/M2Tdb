...
shinyServer(function(input, output) {
  ...
  output$mymapping = DT::renderDataTable(DT::datatable({ 
    ...
    if(input$geneid != "" ) {
      data <- data[which(data$funcGene == trim(input$geneid)),]
    }
    if(input$psmr != "" ) {
    ...
    names(data)=c("MeProbe","MeProbe chr","MeProbe gene","MeProbe BP","eProbe","eProbe chr","eProbe gene","eProbe BP","pSMR","pHEIDI")
